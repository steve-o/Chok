...
static const char* kSnmpApplicationName = "Chok";

LONG volatile chok::snmp_agent_t::ref_count_ = 0;

class chok::snmp::event_pump_t
{
public:
...
chok::snmp_agent_t::snmp_agent_t (chok::chok_t& chok) :
	chok_ (chok)
...
chok::snmp_agent_t::~snmp_agent_t (void)
{
	Clear();
}

bool
chok::snmp_agent_t::Run (void)
{
/* Instance already running. */
	if (InterlockedExchangeAdd (&ref_count_, 1L) > 0)
		return true;

	if (chok_.config_.is_agentx_subagent)
	{
		LOG(INFO) << "Configuring as SNMP AgentX sub-agent.";
		if (!chok_.config_.agentx_socket.empty())
		{
			LOG(INFO) << "Using AgentX socket " << chok_.config_.agentx_socket << '.';
	...
	if (!chok_.config_.snmp_filelog.empty()) {
